...
# env.hosts = ['144.115.201.99']
env.hosts = ['120.79.192.177']

db_user = 'www-date'
db_password = 'www-date'
...
def build():
    ...
    local(r'rm -f dist\%s' % _TAR_FILE)
    with lcd(os.path.join(_current_path(), 'www')):
        cmd = ['tar', '--dereference', '-czvf', '../dist/%s' % _TAR_FILE]
        cmd.extend([r"--exclude='%s'" % ex for ex in excludes])
        cmd.extend(includes)
        local(' '.join(cmd))
...
def rollback():
    ...
    with cd(_REMOTE_BASE_DIR):
        ...
        files.sort(cmp=lambda sl, s2: l if sl < s2 else -1)
        r = run('ls -l www')
        ss = r.split(' -> ')
        if len(ss) != 2:
            ...
            return
        current = ss[1]
        print('Found current synbol link points to: %s\n' % current)
        try:
...
def restore2local():
    ...
    print('')
    try:
        num = int(input ('Restore files: '))
    except ValueError:
        print('Invalid file number.')
        return
    restore_file = files[num]
    yn = input('Restore file %s: %s? y/N ' % (num, restore_file)).lower()
    if yn != 'y':
        print('Restore cancelled.')
        return
    print('Start restore to local databaes...')
